...
include("./StellarFeedback.jl")
using .StellarFeedback
using QuadGK
...
function Pdot_SN(M,z,λ,Z,t)
    """ The total momentum injection rate due to SNe
        at time t (yrs) in a given halo.  """
    ...
    if Z < Zcrit
        ...
        # Momentum injection rate from Pop II SNe:
        dt = 1e5
        SFR_II(tf)    = dMstardt(M,z,λ,Z,tf,dt)  # In Msun/yr
        SN_rateII(tf) = R_CCSN_II(t-tf)          # In # SN/Msun/yr
        integrand(tf) = SFR_II(tf)*SN_rateII(tf)*P_SN(t-tf,n,Z)

        Pdot_SN = quadgk(integrand, 0.0, t, rtol = 1e-3)[1]/yr
    end

    return Pdot_SN
end

#M = 4e7; z = 13.5; λ = 0.03; Z = 0.01; t = 4e6
#println( Pdot_SN(M,z,λ,Z,t) )

function Fgrav(M,z,λ,Z,t)
    """ The gravitational binding force of the gas to 
        the disk. """

    MDisk = Mdisk(M,z,t)
    MGas  = MDisk-Mstar(M,z,λ,Z,t)

    Fgrav = G*MDisk*MGas*(Msun^2)/(Rdisk(M,z,λ,t)^2)

    return Fgrav
end

#println( Fgrav(M,z,λ,Z,t))
...
function Starburst(M,z,λ,Z)
    ...
    while Pdot_SN(M,z,λ,Z,t) < (Mdisk(M,z,t)*Msun/(t*yr))*vdisk(M,z,λ,t) && t < 30e6     
        t += dt
        println(t)
    end
    ...
    return [ tburst/1e6, Mstar(M,z,λ,Z,t) ]

end



M = 5e7; z = 9.0; λ = 0.03; Z = 0.01 #; t = 5e6
println( Starburst(M,z,λ,Z) )
#println( Mach(M,z,λ,Z,t) )
#println(  Q_Toomre(M,z,λ,Z,t) )
#println( Mstar(M,z,λ,Z,t) )