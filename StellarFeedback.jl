...
# and create an interpolation for the SN energy (in 10^51 erg):

CCSN_data     = readdlm("Nomoto2013_SNyieldsAndEnergies_CCSN_11to40Msun.txt")
m_CCSN_data   = CCSN_data[2,2:9]
E_CCSN_data   = CCSN_data[3,2:9]
E_SNIII_CCSN  = Spline1D(m_CCSN_data, E_CCSN_data)
...
E_PISN_data   = PISN_data[3,2:7]
E_SNIII_PISN  = Spline1D(m_PISN_data, E_PISN_data)
...
##################### MOMENTUM PER SUPERNOVA #######################

function P_SNt(n,Z)
    """ The terminal momentum imparted by a single SN
        in a medium of gas number density n (cm^-3) and 
        metallicity Z (relative to Solar). 
        
        From Hopkins et al. (2023) """
    ...
    Zbar  = max(0.01, Z)
    P_SNt = 4.8e5*E51*(n^(-1/7))*(Zbar^(-0.12))*Msun*kms
...
function P_SN(t,n,Z)
    """ Momentum injected into the ISM after a time t (yrs)
        following a single SN, in a medium of gas number
        density n (cm^-3) and metallicity Z (relative to Solar). """
    ...
    # Cooling time (when p_SN = p_t) in years:

    Zbar  = max(0.01, Z)
    tcool = 4.29e4*(E51^(1/3))*(n^(-4/7))*(Zbar^(-1/5))

    # Time-dependent momentum input:

    if t > 0.0
        P_SN = P_SNt(n,Z)*max( (t/tcool)^(3/5), 1.0 )
    else
        P_SN = 0.0
    end
